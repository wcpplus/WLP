 ...
 */
@Repository
public class EvaluationuserDaoImpl extends HibernateSQLTools<EvaluationUser>implements EvaluationuserDaoInter {
	...
	public EvaluationUser getEntity(String evaluationuserid) {
		// TODO 自动生成代码,修改后请去除本注释
		Session session = sessionFatory.getCurrentSession();
		return (EvaluationUser) session.get(EvaluationUser.class, evaluationuserid);
	}
	...
	public int sumScore(String objid) {
		Session session = sessionFatory.getCurrentSession();
		SQLQuery sqlquery = session.createSQLQuery("select sum(score) from wlp_s_evaluation_user where OBJID=?");
		BigDecimal num = (BigDecimal) sqlquery.setString(0, objid).list().get(0);
		return num.intValue();
	}
}
