...
<div style="width: 100%; background-color: #f8fafc; margin-top: 20px;">
	...
	<div class="wlp-userspace-h2">专业管理</div>
	<hr class="wlp-userspace-hr">
	<div class="wlp-usermenu-list">
		<ul>
			<!--待发布专业:tempclass  -->
			<c:if test="${param.current=='tempmajor' }">
				<li class="active">待发布专业<i
					class="glyphicon glyphicon-menu-right"></i></li>
			</c:if>
			<c:if test="${param.current!='tempmajor' }">
				<li class="waiting" onclick="gotoUrl('userspace/tempmajor.do')">待发布专业<i
					class="glyphicon glyphicon-menu-right"></i></li>
			</c:if>
			<!-- 已发布专业:publicclass -->
			<c:if test="${param.current=='publicmajor' }">
				<li class="active">已发布专业<i
					class="glyphicon glyphicon-menu-right"></i></li>
			</c:if>
			<c:if test="${param.current!='publicmajor' }">
				<li class="waiting" onclick="gotoUrl('userspace/publicmajor.do')">已发布专业<i
					class="glyphicon glyphicon-menu-right"></i></li>
			</c:if>
		</ul>
	</div>
