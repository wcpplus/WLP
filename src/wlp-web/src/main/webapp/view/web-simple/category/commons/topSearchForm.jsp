		...
		<ul class="wlp-typeline-ul">
			...
			<c:forEach items="${alltypes}" var="node">
					<c:if test="${node.id==roottypeid }">
						<li class="active"
							onclick="dosearchViewClass(1,'${node.id}','${difficulty}');">${node.text }<span class="wlp-type-falg">${node.num>0?node.num:''}</span></li>
					</c:if>
					<c:if test="${node.id!=roottypeid }">
						<li onclick="dosearchViewClass(1,'${node.id}','${difficulty}');">${node.text }<span class="wlp-type-falg">${node.num>0?node.num:''}</span></li>
					</c:if>
			</c:forEach>
		</ul>
	</div>
