 ...
 */
@Repository
public class MajorchapterDaoImpl  extends HibernateSQLTools<MajorChapter> implements  MajorchapterDaoInter {
  ...
  public MajorChapter getEntity(String majorchapterid) {
    // TODO 自动生成代码,修改后请去除本注释
    Session session= sessionFatory.getCurrentSession();
    return (MajorChapter)session.get(MajorChapter.class, majorchapterid);
  }
  @Override
  public MajorChapter insertEntity(MajorChapter majorchapter) {
    // TODO 自动生成代码,修改后请去除本注释
    Session session= sessionFatory.getCurrentSession();
    session.save(majorchapter);
    return majorchapter;
  }
  @Override
