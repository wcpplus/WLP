...
public class UserclassController extends WebUtils{
  ...
  public Map<String, Object> queryall(DataQuery query,
    ...
    try {
      ...
      DataResult result = userClassServiceImpl
          .createUserclassSimpleQuery(query).search();
  ...
  public Map<String, Object> editSubmit(UserClass entity, HttpSession session) {
        // TODO 自动生成代码,修改后请去除本注释
    try {
      entity = userClassServiceImpl.editUserclassEntity(entity,
  ...
  public Map<String, Object> delSubmit(String ids, HttpSession session) {
    try {
      for (String id : parseIds(ids)) {
        userClassServiceImpl.deleteUserclassEntity(id,
            getCurrentUser(session));
      }
  ...
  public ModelAndView index(HttpSession session) {
    return ViewMode.getInstance()
        .returnModelAndView("learn/UserclassResult");
  }
        /**
   * 显示详细信息（修改或浏览时）
   *
