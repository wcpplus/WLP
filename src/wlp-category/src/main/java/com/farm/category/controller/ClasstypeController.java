...
public class ClasstypeController extends WebUtils {
	...
	public Map<String, Object> queryall(DataQuery query, HttpServletRequest request) {
		try {
			...
			result.runDictionary("3:内容,1:结构", "TYPE"); 
	...
	public Object examtypeTree(String id) {
		try {
			if (id == null) {
				...
				EasyUiTreeNode nodes = new EasyUiTreeNode("NONE", "课程分类", "open", "icon-customers");
				nodes.setChildren(
	 ...
	 * @return
	 */
	@RequestMapping("/freshnums")
	@ResponseBody
	public Map<String, Object> freshdocs(String ids, HttpSession session) {
		try {
			if (StringUtils.isBlank(ids)) {
				if (getCurrentUser(session) != null && !getCurrentUser(session).getType().equals("3")) {
					throw new RuntimeException("当前用户必须选择分类后刷新!");
				}
				for (ClassType type : classTypeServiceImpl.getAllTypes()) {
					classTypeServiceImpl.refreshNum(type.getId());
				}
			} else {
				for (String typeid : parseIds(ids)) {
					List<ClassType> types = classTypeServiceImpl.getSubTypes(typeid,true);
